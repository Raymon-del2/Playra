  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 text-zinc-900 w-full max-w-full overflow-x-hidden pb-24 lg:pb-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <Link
          href="/"
          className="px-4 py-2 rounded-full bg-zinc-200/80 hover:bg-zinc-300/80 text-sm border border-zinc-200 transition-colors"
        >
          Browse videos
        </Link>
      </div>
      ...
      {comments.length > 0 ? (
        <div className="space-y-4">
          {comments.map((comment) => (
            <div
              ...
              className="bg-[#1a1a1a] rounded-xl border border-zinc-200 p-4 hover:border-zinc-200 transition-colors"
            >
              {/* Video Info */}
              <Link
                href={comment.is_short ? `/styles/${comment.video_id}` : `/watch/${comment.video_id}`}
                className="flex gap-3 mb-4 pb-4 border-b border-zinc-200"
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium text-zinc-900 line-clamp-2">{comment.video_title}</h3>
                  <p className="text-xs text-zinc-500 mt-1">by {comment.video_channel_name}</p>
                  <span className="inline-block mt-2 text-[10px] px-2 py-0.5 rounded-full bg-zinc-200/80 text-zinc-500 uppercase tracking-wide">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-medium text-zinc-900">{activeProfile?.name || 'You'}</span>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-24 h-24 rounded-full bg-zinc-200 flex items-center justify-center mb-6">
          ...
          <p className="text-zinc-500 text-lg mb-2">No comments yet</p>
