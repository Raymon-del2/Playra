        ...
        >
            <div className={`w-full max-w-6xl mx-auto flex flex-col items-center transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
                ...
                <div className="flex flex-wrap items-center justify-center gap-10 sm:gap-14 w-full">
                    {profiles.map((profile, i) => (
                        ...
                        >
                            <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full p-[4px] bg-gradient-to-tr from-transparent to-transparent group-hover:from-blue-500 group-hover:to-purple-500 transition-all duration-500 relative ring-1 ring-white/5 shadow-2xl">
                                <div className="w-full h-full rounded-full overflow-hidden border-4 border-zinc-100 group-hover:border-black transition-all">
                                    {profile.avatar ? (
                            ...
                            <div className="flex flex-col items-center gap-1 text-center">
                                <span className="text-zinc-500 font-bold text-lg group-hover:text-zinc-900 transition-colors capitalize">{profile.name}</span>
                                <span className="text-[10px] font-black text-zinc-600 uppercase tracking-tighter group-hover:text-zinc-500 transition-colors">{profile.account_type || 'general'}</span>
                            </div>
                        </div>
                    ))}
                    ...
                    >
                        <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-zinc-100/40 border-2 border-dashed border-zinc-300/50 flex items-center justify-center group-hover:border-white/20 group-hover:bg-zinc-200/80 transition-all">
                            <svg className="w-8 h-8 text-zinc-700 group-hover:text-zinc-900 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4.5v15m7.5-7.5h-15" /></svg>
                        </div>
                        <span className="text-zinc-600 font-bold text-sm tracking-widest uppercase group-hover:text-zinc-500">New</span>
                    </div>
                </div>
            </div>
        </div>
